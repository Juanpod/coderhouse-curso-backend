...
class Contenedor{
    ...
    async save(Object){
       ...
       try {
            try{
                ...
                let data = await readFile(`${this.archivo}.txt`, "utf-8"); 
                ...
                await writeFile(`${this.archivo}.txt`,JSON.stringify([],null,2));
            }
            let data = await readFile(`${this.archivo}.txt`, "utf-8");
            if (data ==""){
                try{
                    await writeFile(`${this.archivo}.txt`,JSON.stringify([],null,2));
                    data = await readFile(`${this.archivo}.txt`, "utf-8")
                }
                catch(error){
                    console.log(error);
                }
    ...
    async updateById(id, body){
        try {
            ...
            await writeFile(`${this.archivo}.txt`,JSON.stringify(productos,null,2));
            return productos;
        } catch (error) {
